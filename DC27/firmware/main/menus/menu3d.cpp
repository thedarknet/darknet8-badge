...
void Menu3D::initMenu3d() {
	CanvasHeight = DN8App::get().getCanvasHeight()-15;
	CanvasWidth = DN8App::get().getCanvasWidth()-30;
...
BaseMenu::ReturnStateContext Menu3D::onRun() {
	...
	if(DN8App::get().getButtonInfo().wasAnyButtonReleased()) {
		sr.NextMenuToRun = DN8App::get().getMenuState();
	}
...
void Menu3D::line(int x0, int y0, int x1, int y1, RGBColor& color) {
	...
	for (int x = x0; x <= x1; x++) {
		...
		if (x >= 0 && x < CanvasWidth && y >= 0 && y < CanvasHeight) {
			if (steep) {
				DN8App::get().getDisplay().drawPixel(y, x, color);
			} else {
				DN8App::get().getDisplay().drawPixel(x, y, color);
			}
		}
	}
}
...
void Menu3D::render() {
	DN8App::get().getDisplay().fillRec(0, 0,CanvasWidth,CanvasHeight,RGBColor::BLACK);
	...
	if (FreeRTOS::getTimeSinceStart() - renderTime > 1000) {
		...
		DN8App::get().getDisplay().drawString(0, DN8App::get().getLastCanvasHeightPixel()-12, &buf[0]);
		count = 0;
