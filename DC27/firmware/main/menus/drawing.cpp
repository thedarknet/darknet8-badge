...
ErrorType DrawingMenu::onInit() {
	...
	uint32_t size = DN8App::get().getBackBufferSize();
	et = NVSDrawing.getBlob("s1", DN8App::get().getBackBuffer() , size);
...
BaseMenu *DrawingMenu::processVirtualButtons(Point2Ds &touchPoint) {
	...
	if(touchPoint.getY()>=14&&touchPoint.getY()<=28) {
		if(NVSDrawing.setBlob("s1",DN8App::get().getBackBuffer(),DN8App::get().getBackBufferSize()).ok()) {
				NVSDrawing.commit();
