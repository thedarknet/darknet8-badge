...
uint8_t WiFi::connectAP(const std::string& ssid, const std::string& password) { 
	return connectAP(ssid,password,0);
}
/**
 ...
 */
uint8_t WiFi::connectAP(const std::string& ssid, const std::string& password, const uint8_t *bssid){
	ESP_LOGI(LOG_TAG, ">> connectAP");

	//check to see if we are already connected to the AP in question?
	if(bssid!=0) {
		wifi_ap_record_t myrec;
		esp_err_t ret = esp_wifi_sta_get_ap_info(&myrec);
		if(ret==ESP_OK) { //we are connected
			if(memcmp(&myrec.bssid[0],bssid,6)==0) { //if we are already connected to this bssid id 
				return ESP_OK;
			} else { //else disconnect and re-connect
				esp_wifi_disconnect();
			}
		}
	}
...
WiFiEventHandler* WiFi::getWifiEventHandler() {
  	return this->m_pWifiEventHandler;
}
